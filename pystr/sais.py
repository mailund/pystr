...
def reduce_LMS(x: isseq, sa: imseq, is_S: bv.BitVector) -> tuple[imseq, imseq, int]:
    ...
    print(compact.i, compact.j, buffer.i, buffer.j)
    buffer[:] = UNDEFINED
    ...
    for j in compact:
        ...
        if not equal_LMS(x, is_S, prev, j):
            letter += 1
        buffer[j//2] = letter
        print(j//2, len(buffer))
        assert 0 <= j//2 < len(buffer)
        assert j//2 not in seen
        seen.add(j//2)
        prev = j
    assert len(seen) == len(compact)
...
def sais_rec(x: isseq, sa: imseq, asize: int, is_S: bv.BitVector):
    print('=' * 10, "sais rec:", x, asize)
    assert max(x) + 1 == asize
    assert set(x) == set(range(asize))
    ...
    if len(x) == asize:
        ...
        for i in range(len(x)):
            assert i in sa, \
                f"{i} class {is_S[i]} is missing"

        assert UNDEFINED not in sa
